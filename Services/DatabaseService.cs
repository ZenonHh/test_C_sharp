...
public class DatabaseService
{
    ...
    // 8. HÀM CẬP NHẬT THÔNG TIN NGƯỜI DÙNG (Overload mới - cập nhật toàn bộ User object)
    public async Task<bool> UpdateUserAsync(int userId, User updatedUser)
    {
        await InitAsync();
        updatedUser.Id = userId;
        var result = await _connection!.UpdateAsync(updatedUser);
        return result > 0;
    }

    // HÀM CẬP NHẬT THÔNG TIN NGƯỜI DÙNG (Overload cũ - cập nhật từng trường)
